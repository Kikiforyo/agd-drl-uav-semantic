...
from agents.td3.agent import TD3Agent, TD3Config
...
def main() -> None:
    ...
    if algo_name == "ddpg":
        ...
        td3_cfg = TD3Config(
            ...
            start_steps=to_int(cfg.get("start_steps", 1000), 1000),
            noise_std=to_float(cfg.get("noise_std", 0.1), 0.1),
            policy_noise=to_float(cfg.get("policy_noise", 0.2), 0.2),
            noise_clip=to_float(cfg.get("noise_clip", 0.5), 0.5),
            policy_delay=to_int(cfg.get("policy_delay", 2), 2),
        )
        agent = TD3Agent(td3_cfg)
        print("[INFO] Starting TD3 training...")
        agent.train(env, num_episodes=td3_cfg.max_episodes, max_steps=td3_cfg.max_steps,
                    noise_start=cfg.get("noise_start", 0.1), noise_end=cfg.get("noise_end", 0.01))
    else:
